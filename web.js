...
async function initializeServer() {
    ...
    try {
        ...
        const mergedData3d = salesData3d.salesvolume.map(item => ({
            ...item,
            calculated_total_price: parseInt(item.product_price.replace(/,/g, ''), 10) * item.total_sales
        })).sort((a, b) => b.calculated_total_price - a.calculated_total_price).slice(0, 14);

        const mergedData6d = salesData6d.salesvolume.map(item => ({
            ...item,
            calculated_total_price: parseInt(item.product_price.replace(/,/g, ''), 10) * item.total_sales
        })).sort((a, b) => b.calculated_total_price - a.calculated_total_price).slice(0, 14);
